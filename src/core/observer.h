...
public:
    ...
    bool addObserver(ObserverPtr observer) {
        ...
        qDebug() << QString("Subject: 添加观察者") << observer->getObserverName() 
                 << QString("，当前观察者数量:") << m_observers.size();
        return true;
    }
    ...
    bool removeObserver(ObserverPtr observer) {
        ...
        if (removed) {
            qDebug() << QString("Subject: 移除观察者") << observer->getObserverName()
                     << QString("，当前观察者数量:") << m_observers.size();
        }
    ...
    void notifyObservers(const EventType& event) {
        ...
        for (const auto& observer : validObservers) {
            try {
                // 直接调用观察者方法，避免QMetaObject::invokeMethod的lambda问题
    ...
    void clearObservers() {
        QMutexLocker locker(&m_mutex);
        m_observers.clear();
        qDebug() << QString("Subject: 清理所有观察者");
    }
