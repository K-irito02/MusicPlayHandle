...
{
    // CancellableTask不是QObject，不能使用qobject_cast
    // 这里需要重新设计任务完成的通知机制
    // 暂时注释掉这个实现
    /*
...
void ThreadPoolManager::updateGlobalStatistics()
{
    ...
    int totalThreads = 0;
    for (auto it = m_pools.begin(); it != m_pools.end(); ++it) {
        totalThreads += it->second->maxThreadCount();
    }
...
{
    // 创建专用线程池
    m_pools.emplace(AudioProcessingPool, std::make_unique<SmartThreadPool>());
    m_pools.emplace(DecodingPool, std::make_unique<SmartThreadPool>());
    m_pools.emplace(FileIOPool, std::make_unique<SmartThreadPool>());
    m_pools.emplace(GeneralPool, std::make_unique<SmartThreadPool>());
    
    // 配置每个池
    for (auto it = m_pools.begin(); it != m_pools.end(); ++it) {
        PoolType type = it->first;
        SmartThreadPool* pool = it->second.get();
        
        configurePool(type, pool);
        
        // 连接统计信号
        connect(pool, &SmartThreadPool::poolStatisticsUpdated,
