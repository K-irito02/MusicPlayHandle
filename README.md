...
- **描述**: Modern Qt6 Music Player with Advanced Tag Management, FFmpeg Audio Processing and Performance Optimization
- **构建系统**: qmake (主要) / CMake (兼容)
- **Qt版本**: Qt6.5.3+ (MinGW 64-bit)
- **语言标准**: C++17
- **开发环境**: Qt Creator 11.0.0+
- **最后更新**: 2025年07月25日 (架构重构与性能优化)
- **主要功能**: 音频播放、标签管理、播放历史、精确进度控制、界面同步、FFmpeg音频处理、性能优化
- **特色技术**: PreciseSlider、多线程音频处理、播放历史管理、UI控件冲突解决、界面同步机制、FFmpeg实时音频处理、观察者模式架构、智能线程池、性能监控
...
- **对话框模块**
  ...
  - `ImprovedPlayInterface` - 改进的音乐播放界面
...
- **音频引擎**
  ...
  - `ImprovedAudioEngine` - 改进的音频引擎 (观察者模式、资源管理、性能监控)
...
#### 4. **线程管理层**
- **`MainThreadManager`** - 主线程管理器 (线程安全UI更新、批量处理、性能监控)
- **`ThreadPoolManager`** - 智能线程池管理器 (自适应线程数、负载均衡)
- **`AudioWorkerThread`** - 音频工作线程

#### 5. **性能优化层**
- **`PerformanceManager`** - 性能监控管理器 (CPU/内存监控、自适应解码频率)
- **`ResourceManager`** - 资源管理器 (RAII资源管理、内存池、音频资源独占)
- **`ObjectPool`** - 对象池管理 (智能对象复用、内存优化)
- **`Cache`** - LRU缓存系统 (智能缓存策略)
- **`LazyLoader`** - 延迟加载器 (按需加载优化)
...
├── copy_ffmpeg_dlls.bat     # FFmpeg DLL文件复制工具
...
│   │   ├── improvedaudioengine.h/cpp # 改进的音频引擎
...
│   │   ├── observer.h       # 观察者模式接口
│   │   ├── performancemanager.h/cpp # 性能监控管理器
...
│   │   └── threadpoolmanager.h/cpp  # 智能线程池管理
│   ├── ui/                  # 用户界面组件
│   │   ├── controllers/     # UI控制器
...
│   │   │   ├── improvedplayinterface.h/cpp # 改进的播放界面
...
#### FFmpeg DLL文件配置
项目已提供 `copy_ffmpeg_dlls.bat` 批处理工具，用于自动复制FFmpeg DLL文件：
- 自动检测FFmpeg库路径
- 复制所有必要的DLL文件到Debug和Release目录
- 支持错误检查和状态反馈
- 包含以下DLL文件：
  - `avcodec-61.dll` - 音频/视频编解码器
  - `avdevice-61.dll` - 设备接口
  - `avfilter-10.dll` - 音视频过滤器
  - `avformat-61.dll` - 音视频格式处理
  - `avutil-59.dll` - 通用工具库
  - `postproc-58.dll` - 后处理库
...
- [x] **架构重构与性能优化**: 
  - [x] 观察者模式架构重构
  - [x] ImprovedAudioEngine (资源管理、性能监控)
  - [x] ImprovedPlayInterface (线程安全UI更新)
  - [x] MainThreadManager (线程安全UI更新、批量处理)
  - [x] ThreadPoolManager (智能线程池、负载均衡)
  - [x] PerformanceManager (CPU/内存监控、自适应解码)
  - [x] ResourceManager (RAII资源管理、内存池)
  - [x] 音频资源独占机制
  - [x] 动态解码频率调整
  - [x] 智能缓存和对象池优化
  - [x] FFmpeg DLL自动配置工具
